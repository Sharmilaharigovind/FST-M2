...
inputDialogues4 = LOAD 'hdfs:///user/root/my_hive_data/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/root/my_hive_data/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/root/my_hive_data/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/sharmila/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/roots/outputs' USING PigStorage('\t');
